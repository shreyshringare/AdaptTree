...
// split_leaf()
// ─────────────────────────────────────────────────────────────────────────────
// Pre-condition: leaf at leaf_page_id has exactly ORDER+1 entries (over-full).
// Post-condition: left leaf has SPLIT_HALF entries; right leaf has ORDER+1-SPLIT_HALF entries.
// *fence_key_out  = entries[SPLIT_HALF].key  (smallest key in right leaf)
// *right_page_id_out = page ID of newly allocated right leaf.
//
// The leaf_page_id leaf must have ORDER+1 valid entries stored starting at index 0.
// The caller is responsible for having inserted the new entry into a temporary
// buffer and stored all ORDER+1 entries back into the page before calling this.

template <typename WAL_T>
bool BPlusTree<WAL_T>::split_leaf(uint32_t  leaf_page_id,
                                   uint64_t* fence_key_out,
                                   uint32_t* right_page_id_out)
{
    // Snapshot the ORDER+1 entries from the left leaf, then trim left to SPLIT_HALF.
    LeafEntry all_entries[ORDER + 1];
    uint32_t  old_next_leaf = INVALID_PAGE_ID;

    {
        auto left_g = pool_.fetchPage(leaf_page_id);
        if (!left_g) return false;
        auto* left = reinterpret_cast<LeafNode*>(left_g->data());

        assert(left->header.num_slots == ORDER + 1);
        std::memcpy(all_entries, left->entries, (ORDER + 1) * sizeof(LeafEntry));
        old_next_leaf = left->header.next_leaf_id;

        // Trim left leaf to SPLIT_HALF entries.
        left->header.num_slots = static_cast<uint16_t>(SPLIT_HALF);
        left->header.lsn       = wal_.append(leaf_page_id, nullptr, 0);
        left_g->markDirty();
        // left_g destructs here — unpin before newPage()
    }
    ...
    *right_page_id_out       = right_id;
    *fence_key_out           = all_entries[SPLIT_HALF].key;
    ...
    right->header.next_leaf_id = old_next_leaf;
    ...
    // Update left leaf's next_leaf_id to point to the new right leaf.
    {
        auto left_g = pool_.fetchPage(leaf_page_id);
        if (!left_g) return false;
        auto* left              = reinterpret_cast<LeafNode*>(left_g->data());
        left->header.next_leaf_id = right_id;
        left->header.lsn          = wal_.append(leaf_page_id, nullptr, 0);
        left_g->markDirty();
    }
...
// split_internal()
// ─────────────────────────────────────────────────────────────────────────────
// Pre-condition: internal node at node_page_id has ORDER keys and ORDER+1 children
// (one over capacity — the overflow entry has already been inserted).
// The middle key keys[SPLIT_HALF] is pushed up to the parent.
// Left node keeps keys[0..SPLIT_HALF-1] and children[0..SPLIT_HALF].
// Right node gets keys[SPLIT_HALF+1..ORDER-1] and children[SPLIT_HALF+1..ORDER].

template <typename WAL_T>
bool BPlusTree<WAL_T>::split_internal(uint32_t  node_page_id,
                                       uint64_t* push_up_key_out,
                                       uint32_t* right_page_id_out)
{
    // Snapshot all ORDER keys and ORDER+1 children, then trim the left node.
    uint64_t all_keys[ORDER];
    uint32_t all_children[ORDER + 1];
    uint64_t middle_key = 0;

    {
        auto left_g = pool_.fetchPage(node_page_id);
        if (!left_g) return false;
        auto* left = reinterpret_cast<InternalNode*>(left_g->data());

        assert(left->header.num_slots == ORDER);

        std::memcpy(all_keys,     left->keys,     ORDER * sizeof(uint64_t));
        std::memcpy(all_children, left->children, (ORDER + 1) * sizeof(uint32_t));

        middle_key     = all_keys[SPLIT_HALF];
        *push_up_key_out = middle_key;

        // Left keeps keys[0..SPLIT_HALF-1] and children[0..SPLIT_HALF]
        left->header.num_slots = static_cast<uint16_t>(SPLIT_HALF);
        // keys[0..SPLIT_HALF-1] and children[0..SPLIT_HALF] are already in place
        left->header.lsn       = wal_.append(node_page_id, nullptr, 0);
        left_g->markDirty();
    }
    ...
    *right_page_id_out     = right_id;
    ...
    right->header.free_start   = 0;
    right->header.free_end     = 0;
    right->header.next_leaf_id = INVALID_PAGE_ID;

    // Right gets keys[SPLIT_HALF+1..ORDER-1] and children[SPLIT_HALF+1..ORDER]
    uint32_t right_key_count = static_cast<uint32_t>(ORDER - SPLIT_HALF - 1);
    right->header.num_slots  = static_cast<uint16_t>(right_key_count);

    std::memcpy(right->keys,
                &all_keys[SPLIT_HALF + 1],
                right_key_count * sizeof(uint64_t));
    std::memcpy(right->children,
